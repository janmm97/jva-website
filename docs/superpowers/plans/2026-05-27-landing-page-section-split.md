# Landing Page Section Split & Projects Rename — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remove About, Services, Products, and SocialProof from the landing page, give each a dedicated route, and rename Products → Projects throughout.

**Architecture:** Each previously inline section gets its own Next.js page route. The Products component is renamed to Projects (new file, old deleted). SocialProof is removed from all surfaces. Navbar links switch from hash anchors to page routes.

**Tech Stack:** Next.js 13+ App Router, TypeScript, Tailwind CSS, Framer Motion
...
- Delete: `components/sections/Products.tsx`
- Create: `app/projects/page.tsx`
- Delete: `app/products/page.tsx` (and `app/products/` folder)
- Modify: `app/page.tsx`
- Modify: `app/about/page.tsx`
- Modify: `app/services/page.tsx`
- Modify: `components/layout/Navbar.tsx`

**Task order matters:** Create Projects.tsx and app/projects/page.tsx before deleting their Products counterparts, to avoid broken imports during intermediate steps.

---

### Task 1: Create `components/sections/Projects.tsx`
...
Create `components/sections/Projects.tsx` with the following content (same internals as Products.tsx — three targeted changes: function name `Projects`, section id `"projects"`, section label `"Projects"`):
...
const projects = [
  ...
  { status: "Coming Soon",   statusVariant: "warning" as const, title: "Automated Social Media Content Pipeline", description: "A full team of AI agents — content strategist, script writer, prompt engineer, and video director — working together to produce scroll-stopping videos from scratch. Your brand voice goes in, ready-to-post content comes out.",                             preview: <UGCPreview />,         tools: ["Notion","OpenRouter","ElevenLabs","HeyGen","Kie AI"],      cta: "Join waitlist →", ctaHref: BOOKING_URL,                                   comingSoon: true  },
...
export function Projects() {
  ...
  return (
    <section ref={sectionRef} id="projects" className="relative min-h-screen flex items-center py-20 overflow-hidden bg-jva-dark">
...
git commit -m "feat: add Projects component (renamed from Products)"
```

---

### Task 2: Create `app/projects/page.tsx`
...
git commit -m "feat: add /projects page route"
```

---

### Task 3: Delete `app/products/page.tsx`

**Files:**
- Delete: `app/products/page.tsx`

- [ ] **Step 1: Delete the file**

```bash
git rm app/products/page.tsx
```

- [ ] **Step 2: Commit**

```bash
git commit -m "chore: delete /products route (replaced by /projects)"
```

---

### Task 4: Delete `components/sections/Products.tsx`
...
```bash
git rm components/sections/Products.tsx
```

- [ ] **Step 2: Verify no remaining imports of Products**

```bash
grep -r "from.*sections/Products" app/ components/
```
...
```bash
git commit -m "chore: delete Products component (replaced by Projects)"
```

---

### Task 5: Update Navbar links
...
In `components/layout/Navbar.tsx`, find the `navLinks` array and replace it with:
...
git commit -m "feat: update navbar links to page routes, rename Products to Projects"
```

---

### Task 6: Remove sections from landing page
...
git commit -m "feat: remove About, Services, Products, SocialProof from landing page"
```

---

### Task 7: Simplify About page

**Files:**
- Modify: `app/about/page.tsx`

- [ ] **Step 1: Replace `app/about/page.tsx` entirely**
...
git commit -m "feat: simplify About page — remove header and FinalCTA"
```

---

### Task 8: Remove SocialProof from Services page

**Files:**
- Modify: `app/services/page.tsx`

- [ ] **Step 1: Replace `app/services/page.tsx` entirely**
...
git commit -m "feat: remove SocialProof from Services page"
```
...
Expected: `✓ Compiled successfully` with routes `/`, `/about`, `/services`, `/projects` all listed. `/products` should not appear.
...
Manually verify:
- `/` — Hero, IntegrationSphere, Process, Pricing, FinalCTA (no About/Services/Products/SocialProof)
- `/about` — About section only (no page header banner, no FinalCTA)
- `/services` — page header + Services, Process, Pricing, FinalCTA (no SocialProof)
- `/projects` — page header + Projects grid + IntegrationSphere + FinalCTA
- `/products` — returns 404
- Navbar shows: Home, Services, Projects, About (all as page links, not hash anchors)
